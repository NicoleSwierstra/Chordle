...
public class GUI {
    ...
    public void onMouse(float x, float y){
        Elements.forEach(b -> {
            if(b instanceof Button || b instanceof TextBox || b instanceof ToggleButton)
    ...
    public ToggleButton queueToggleButton(String label, float x, float y, float width, float height, ToggleButtonInterface tbi){
        ToggleButton tb = new ToggleButton(label, x, y, width, height, tbi); eQueue.add(tb); return tb;
    }
    ...
    boolean checkIntersect(GUIElement b, boolean press, float x, float y){
        ...
        if(isx && isy){
            if(press) {
                ...
                else if(b instanceof ToggleButton){
                    ToggleButton tb = (ToggleButton)b;
                    tb.toggled = !tb.toggled;
                    tb.tbi.onClick(tb.toggled);
                }
